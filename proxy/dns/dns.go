...
import (
	...
	dns_proto "github.com/gzjjjfree/gzv2ray-v4/common/protocol/dns"
...
func (h *Handler) Process(ctx context.Context, link *transport.Link, d internet.Dialer) error {
	...
	newError("handling DNS traffic to ", dest)
...
func (c *outboundConn) Close() error {
	fmt.Println("in proxy-dns-dns.go func (c *outboundConn) Close()")
	c.access.Lock()
	close(c.connReady)
	if c.conn != nil {
		c.conn.Close()
