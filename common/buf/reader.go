...
func readOneUDP(r io.Reader) (*Buffer, error) {
	fmt.Println("in common-buf-reader.go func readOneUDP")
	b := New()
	for i := 0; i < 64; i++ {
		_, err := b.ReadFrom(r)
		if !b.IsEmpty() {
...
func (r *BufferedReader) Read(b []byte) (int, error) {
	//fmt.Println("in common-buf-reader.go func Read(b []byte)")
	spliter := r.Spliter
	if spliter == nil {
		spliter = SplitBytes
	}
...
func (r *BufferedReader) writeToInternal(writer io.Writer) (int64, error) {
	fmt.Println("in common-buf-reader.go func writeToInternal(writer io.Writer)")
...
func (r *BufferedReader) WriteTo(writer io.Writer) (int64, error) {
	fmt.Println("in common-buf-reader.go func WriteTo(writer io.Writer)")
...
func (r *BufferedReader) Interrupt() {
	fmt.Println("in common-buf-reader.go func Interrupt()")
	common.Interrupt(r.Reader)
}

// Close implements io.Closer.
func (r *BufferedReader) Close() error {
	fmt.Println("in common-buf-reader.go func Close()")
...
func (r *PacketReader) ReadMultiBuffer() (MultiBuffer, error) {
	fmt.Println("in common-buf-reader.go func  (r *PacketReader)  ReadMultiBuffer()")
