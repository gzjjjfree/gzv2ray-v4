...
func New(ctx context.Context, config *Config) (*DNS, error) {
	...
	default:
		return nil, errors.New("unexpected client IP length")
	}
	...
	if err != nil {
		return nil, errors.New("failed to create hosts")
	}
...
func (s *DNS) Start() error {
	fmt.Println("in  app-dns-dns.go func Start()")
	return nil
}

// Close implements common.Closable.
func (s *DNS) Close() error {
	fmt.Println("in app-dns-dns.go func (s *DNS) Close()")
	return nil
}
...
func (s *DNS) lookupIPInternal(domain string, option dns.IPOption) ([]net.IP, error) {
	...
	case len(addrs) == 1 && addrs[0].Family().IsDomain(): // Domain replacement
		fmt.Println("domain replaced: ", domain, " -> ", addrs[0].Domain())
		domain = addrs[0].Domain()
	default: // Successfully found ip records in static host
		fmt.Println("returning ", len(addrs), " IPs for domain ", domain)
	...
	for _, client := range s.sortClients(domain) {
		if !option.FakeEnable && strings.EqualFold(client.Name(), "FakeDNS") {
			fmt.Println("skip DNS resolution for domain ", domain, " at server ", client.Name())
			continue
		}
		...
		if err != nil {
			...
			fmt.Println("failed to lookup ip for domain ", domain, " at server ", client.Name(), " errs: ", errs)
		}
	...
	return nil, errors.New("returning nil for domain")
}
...
func (s *DNS) sortClients(domain string) []*Client {
	...
	if len(domainRules) > 0 {
		fmt.Println("domain ", domain, " matches following rules: ", domainRules)
	}
	if len(clientNames) > 0 {
		fmt.Println("domain ", domain, " will use DNS in order: ", clientNames)
	}

	if len(clients) == 0 {
		clients = append(clients, s.clients[0])
		clientNames = append(clientNames, s.clients[0].Name())
		fmt.Println("domain ", domain, " will use the first DNS: ", clientNames)
	}
