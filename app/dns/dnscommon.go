...
func parseResponse(payload []byte) (*IPRecord, error) {
	...
	if err != nil {
		return nil, errors.New("failed to parse DNS response")
	}
	if err := parser.SkipAllQuestions(); err != nil {
		return nil, errors.New("failed to skip questions in DNS response")
	}
	...
	for {
		...
		if err != nil {
			if err != dnsmessage.ErrSectionDone {
				fmt.Println("failed to parse answer section for domain: ", ah.Name.String())
			}
			break
		}
		...
		case dnsmessage.TypeA:
			...
			if err != nil {
				fmt.Println("failed to parse A record for domain: ", ah.Name)
				break L
			}
		...
		default:
			if err := parser.SkipAnswer(); err != nil {
				fmt.Println("failed to skip answer")
				break L
			}
			continue
		}
