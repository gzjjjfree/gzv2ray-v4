...
func (f *Field) packVal(buf []byte, val reflect.Value, length int, options *Options) (size int, err error) {
	...
	default:
		panic(fmt.Sprintf("no pack handler for type: %s %v", typ, length))
	}
	return
}
...
func (f *Field) unpackVal(buf []byte, val reflect.Value, length int, options *Options) error {
	...
	case Float32, Float64:
		...
		case reflect.Float32, reflect.Float64:
			val.SetFloat(n)
		default:
			return fmt.Errorf("struc: refusing to unpack float into field %s of type %s %v", f.Name, f.kind.String(), length)
		}
	case Bool, Int8, Int16, Int32, Int64, Uint8, Uint16, Uint32, Uint64:
		var n uint64
		switch typ {
