...
func (h *DynamicInboundHandler) closeWorkers(workers []worker) {
	...
	for idx, worker := range workers {
		...
		if err := worker.Close(); err != nil {
			errors.New("failed to close worker")
		}
	}
...
func (h *DynamicInboundHandler) refresh() error {
	...
	for i := uint32(0); i < concurrency; i++ {
		...
		if err != nil {
			errors.New("failed to create proxy instance")
			continue
		}
		...
		if net.HasNetwork(nl, net.Network_TCP) {
			...
			if err := worker.Start(); err != nil {
				errors.New("failed to create TCP worker")
				continue
			}
		...
		if net.HasNetwork(nl, net.Network_UDP) {
			...
			if err := worker.Start(); err != nil {
				errors.New("failed to create UDP worker")
				continue
			}
			workers = append(workers, worker)
		}
...
func (h *DynamicInboundHandler) Start() error {
	fmt.Println("in app-proxyman-inbound-dynamic.go func Start()")
	return h.task.Start()
}

func (h *DynamicInboundHandler) Close() error {
	fmt.Println("in app-proxyman-inbound-dynamic.go func (h *DynamicInboundHandler) Close() ")
	return h.task.Close()
}
