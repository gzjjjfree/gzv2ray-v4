...
import (
	...
	routing_session "github.com/gzjjjfree/gzv2ray-v4/features/routing/session"
...
func (*DefaultDispatcher) Start() error {
	fmt.Println("in app-dispatcher-default.go func Start()")
	return nil
}
...
func (d *DefaultDispatcher) routedDispatch(ctx context.Context, link *transport.Link, destination net.Destination) {
	...
	if forcedOutboundTag := session.GetForcedOutboundTagFromContext(ctx); forcedOutboundTag != "" {
		...
		if h := d.ohm.GetHandler(forcedOutboundTag); h != nil {
			...
			newError("non existing tag for platform initialized detour: ", forcedOutboundTag).AtError()
		...
		if route, err := d.router.PickRoute(routing_session.AsRoutingContext(ctx)); err == nil {
			...
			newError("default route for ", destination)
		}
	}
	...
	if handler == nil {
		newError("default outbound handler not exist")
		common.Close(link.Writer)
		common.Interrupt(link.Reader)
		return
	}
