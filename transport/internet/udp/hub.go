...
import (
	...
	"github.com/gzjjjfree/gzv2ray-v4/common/protocol/udp"
...
func (h *Hub) Close() error {
	fmt.Println("in transport-internet-udp-hub.go func (h *Hub) Close()")
	h.conn.Close()
	return nil
}
...
func (h *Hub) start() {
	fmt.Println("in transport-internet-udp-hub.go func (h *Hub) start()")
	c := h.cache
	defer close(c)
	...
	for {
		...
		if err != nil {
			fmt.Println("failed to read UDP msg err is: ", err)
			buffer.Release()
			break
		}
		...
		if h.recvOrigDest && noob > 0 {
			...
			if payload.Target.IsValid() {
				fmt.Println("UDP original destination: ")
			} else {
				fmt.Println("failed to read UDP original destination")
			}
		}
