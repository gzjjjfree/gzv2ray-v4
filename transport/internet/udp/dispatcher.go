...
func (v *Dispatcher) getInboundRay(ctx context.Context, dest net.Destination) *connEntry {
	...
	fmt.Println("establishing new connection for ", dest)
...
func handleInput(ctx context.Context, conn *connEntry, dest net.Destination, callback ResponseCallback) {
	...
	for {
		...
		if err != nil {
			fmt.Println("failed to handle UDP input")
			return
		}
		timer.Update()
		for _, b := range mb {
...
func (c *dispatcherConn) Close() error {
	fmt.Println("in transport-internet-udp-dispatcher.go func (c *dispatcherConn) Close()  ")
	return c.done.Close()
}
